...
function renderHourlyTable(prediction) {
  ...
  for (const row of prediction.hourly) {
    ...
    const actualValue =
      row.actualTemperature === null ? "-" : formatTemp(row.actualTemperature);
    const absErrValue =
      row.absoluteError === null ? "-" : formatTemp(row.absoluteError);
    tr.innerHTML = `
...
function renderBars(prediction) {
  bars.innerHTML = "";
  const tempValues = prediction.hourly.map((row) =>
    Number(row.predictedTemperature),
  );
  ...
  for (const row of prediction.hourly) {
    const normalized =
      ((Number(row.predictedTemperature) - minTemp) / span) * 100;
...
function renderDetails(prediction) {
  ...
  const cards = [
    ...
    {
      title: "Selected Day",
      stats: prediction.dayMetrics || {
        note: "No actual values available yet",
      },
    },
    {
      title: "Context",
      stats: {
        ...
        model_source: prediction.loadedFromCache
          ? "saved artifact"
          : "fresh training",
  ...
  for (const cardData of cards) {
    ...
    card.innerHTML = `
        ...
        .map(
          ([key, value]) =>
            `<p class="split-stat"><strong>${key.replaceAll("_", " ")}:</strong> ${typeof value === "number" ? value.toFixed(4) : value}</p>`,
        )
        .join("")}
		`;
...
async function handlePrediction(event) {
  ...
  if (!payload.savedModel) {
    throw new Error(
      "No saved model is available. Train and save a Level 2 model first.",
    );
  }

  setStatus("Running temperature prediction...");
  log(
    `Predicting temperature for ${payload.selectedDate} using saved model ${payload.savedModel}.`,
  );
